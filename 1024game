...
# class for 1024 game , init method, display board with cells
...
# initialising board with manual input for some particular cell
...
# updating a initial empty cell with a number 2

  def update_cell(self, cell_no):
    if self.cell[cell_no] == " ":
      self.cell[cell_no] = 2

# now methods for right move possibilities, empty conditions, addition cond
  ...
  def rmove(self,n):
    if self.cell[n] == " ":
      ...
      elif(self.cell[n-1] != " ") and (self.cell[n-2] != " ") and (self.cell[n-1] == self.cell[n-2]):
        self.cell[n] = self.cell[n-1] + self.cell[n-2]
        self.cell[n-1] = " "
        self.cell[n-2] = " "
      
      elif(self.cell[n-1] != self.cell[n-2]):
        self.cell[n] = self.cell[n-1]
      ...
      elif (self.cell[n-1]==self.cell[n]) and (self.cell[n-2] != self.cell[n-1]):
        self.cell[n] = self.cell[n-1] + self.cell[n]
        self.cell[n-1] = self.cell[n-2]
  ...
  def lmove(self,n): 
    if self.cell[n] == " ": 
      ...
      elif (self.cell[n] == self.cell[n+1] ) and (self.cell[n+1] != self.cell[n+2]):
  ...
  def umove(self,n): 
    if self.cell[n] == " ": 
      ...
      elif (self.cell[n+3] == self.cell[n]) and (self.cell[n+6] != self.cell[n+3]):
  ...
  def dmove(self,n): 
    if self.cell[n] == " ": 
      ...
      elif (self.cell[n] == self.cell[n-3]) and (self.cell[n-3] != self.cell[n-6]):
        self.cell[n] = self.cell[n-3] + self.cell[n]
        self.cell[n-3] = self.cell[n-6]
...
select_game = int(raw_input("select the game board to play fro : 4, 8 ,16, 32, 64 :- "))

gameboard.initial_board()
gameboard.display()

while True :
  user_input = raw_input("select the move u for up, d for down , l for left , r for right ")
  ...
  if gameboard.win_check(select_game):
    ...
    if play_again == "Y":
      gameboard.reset()
      select_game = int(raw_input("selest the game board to play for : 4, 8, 16, 32 ,64 :- "))
